describe Taro::Types::Field do
  ...
  it 'falls back to defaults if given' do
    field = described_class.new(name: :foo, type: String, null: false, default: 'bar')
    expect(field.extract_value({})).to eq('bar')
  end

  it 'can use :method to access a custom hash key' do
    field = described_class.new(name: :foo, type: String, null: false, method: :bar)
    expect(field.extract_value({ bar: 'HI' })).to eq('HI')
  end

  it 'can use :method to call a custom method' do
    field = described_class.new(name: :foo, type: String, null: false, method: :upcase)
    expect(field.extract_value('low', from_hash: false)).to eq('LOW')
  end
  ...
  it 'calls :method on the context if defined directly on it' do
    ...
    field = described_class.new(name: :foo, type: String, null: false)
    expect(field.extract_value(:ARG, context:, from_hash: false)).to eq('CTX')
  end

  it 'does not call :method on the context if its an inherited method' do
    context = Class.new(T::ObjectType).new(nil)
    field = described_class.new(name: :foo, type: String, null: false, method: :inspect)
    expect(field.extract_value(:ARG, context:, from_hash: false)).to eq(':ARG')
  end
  ...
  it 'can allow null values' do
    field = described_class.new(name: :foo, type: String, null: true)
  ...
  it 'raises for values of the wrong type' do
    field = described_class.new(name: :foo, type: String, null: false)
    expect do
      field.extract_value(42, from_hash: false)
    end.to raise_error(Taro::RuntimeError, /Failed to coerce/)
  end

  it 'raises for invalid null settings' do
    expect do
      described_class.new(name: :foo, type: String, null: nil)
    end.to raise_error(Taro::ArgumentError, /null/)
  end

  it 'raises for invalid type settings' do
    expect do
      described_class.new(name: :foo, type: Object, null: false)
    end.to raise_error(Taro::ArgumentError, /type/)
  end
end
